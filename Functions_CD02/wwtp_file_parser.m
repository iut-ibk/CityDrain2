reactor=function load_wwtp(filename)
...
while ~isempty(line)
	...
	switch type
		...
		case {'W','w'},
			...
			inflow=zeros(1,r+1);
			concentrations=zeros(1,c);		
			reactor=[0 0 0 0 0 0 inflow concentrations]
		%reactor
		case {'R','r'},
			t=2;
			[tok,rem]=strtok(rem)
	...
	if type~='w' & type~='W'
		reactor=[reactor; t A h lay vol	o2 inflow concentrations];
	end
	n;
	linecount;
